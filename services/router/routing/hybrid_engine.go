...
// 关键词匹配层不需要 ctx（纯内存计算）。
func (r *HybridRouter) Match(ctx context.Context, prompt string, model string) *model.Config {
	if prompt == "" {
		return r.fallback()
	}
...
//   - 最高相似度低于阈值（语义不确定）
//   - 匹配到的意图对应的供应商不可用
func (r *HybridRouter) semanticMatch(ctx context.Context, prompt string) *model.Config {
	var emb Embedding
	var ok bool
...
//   - 分类结果解析失败
//   - 分类结果对应的供应商不可用
func (r *HybridRouter) classify(ctx context.Context, prompt string) *model.Config {
	r.intentMu.RLock()
	clIntents := r.classifyIntents
	r.intentMu.RUnlock()
...
func (r *HybridRouter) fallback() *model.Config {
	if prov, ok := r.providers[r.defaultProvider]; ok && prov.Enabled {
